...
async def fake_network_request(request):
    print(str(os.getppid())+':'+str(os.getpid())+'making network call for request:  ' + request)
...
async def web_server_handler():
    ...
    print(str(os.getppid())+':'+str(os.getpid())+'doing useful work while network calls are in progress...')

    # wait for the network calls to complete. Time to step off the event loop using await!
    await asyncio.wait([task1, task2])

    print(str(os.getppid())+':'+str(os.getpid())+ task1.result())
    print(str(os.getppid())+':'+str(os.getpid())+ task2.result())
